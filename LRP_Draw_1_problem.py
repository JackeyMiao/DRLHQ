...
def Draw_1_Problem(data, solution, result_folder, episode, aug):
    ...
    for l2 in full_dict.values():
        p1.append(l2[0])
        p2.append(l2[1])
    plt.plot(p1[:depot_size], p2[:depot_size], 'g*', ms=5, label='depot')
    plt.plot(p1[depot_size:], p2[depot_size:], 'ko', ms=5, label='customer')
    plt.grid(True)
    ...
    plt.savefig('{}/routes_{}_{}.jpg'.format(result_folder, episode, aug))
    plt.show()
