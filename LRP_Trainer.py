...
from LRP_EnvTrain import LRPEnvTrain
...
class LRPTrainer:
    def __init__(self,
        ...
        # main components
        self.model = LRPModel(**model_params)
        self.env = LRPEnvTrain(**env_params)
    ...
    def train_1_epoch(self, epoch):
        ...
        while episode < self.episodes:
            ...
            score_avg, loss_avg = self.train_1_batch(batch_size, epoch)
    ...
    def train_1_batch(self, batch_size, epoch):
        # prepare
        self.model.train()
        self.env.load_problems(batch_size, device=self.device)
        ...
        last_hh = None
        while not done:
            selected, probability, last_hh = self.model(state, last_hh)
            # shape: (batch_size, mt_size)
            state, reward, done = self.env.step(selected, epoch)
