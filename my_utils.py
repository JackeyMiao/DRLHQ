...
class TimeEstimator:
    ...
    def get_est_string(self, count, total):
        ...
        elapsed_time_str = "{:.4f}h".format(elapsed_time / 3600) if elapsed_time > 3600 else "{:.4f}m".format(elapsed_time / 60) if elapsed_time > 60 else "{:.4f}s".format(elapsed_time)
        remain_time_str = "{:.4f}h".format(remain_time / 3600) if remain_time > 3600 else "{:.4f}m".format(remain_time / 60) if remain_time > 60 else "{:.4f}s".format(remain_time)
